...
export const login = asyncHandler(async (req, res) => {
  ...
  if (!user.is_active)
    return makeResponse({
      ...
      status: 401,
      message:
        "Your account has been deactivated. Please contact an administrator to resolve it",
    });
...
export const forgotPassword = asyncHandler(async (req, res) => {
  ...
  return makeResponse({
    res,
    message:
      "A password registration link has been emailed to you. Please use it to reset your password",
  });
});

export const resetPassword = asyncHandler(async (req, res) => {
  const result = await resetPasswordFromEmail(
    req.body.password,
